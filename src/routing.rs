...
pub async fn read_socket(socket: &mut TcpSocket<'_>) -> Result<RequestType, RoutingError> {
        ...
        match socket.read(&mut buffer).await {
            ...
            Ok(len) => {
                let content = core::str::from_utf8(&buffer[..(pos + len)])
                    .map_err(|_| RoutingError::InvalidUTF8)?;
                pos += len;
                if content.contains("\r\n\r\n") {
                    break;
                }
            ...
            Err(e) => {
                return Err(RoutingError::ReadError(e));
            }
        };
    }
    let content = core::str::from_utf8(&buffer[..(pos)]).map_err(|_| RoutingError::InvalidUTF8)?;
    println!("{}", content);
    ...
    fn try_from(content: &str) -> Result<Self, Self::Error> {
        if content.starts_with("GET ") {
            if content.contains("Upgrade: websocket") {
                ...
                let key_start =
                    content.find(pat).ok_or(RoutingError::InvalidPacketFormat)? + pat.len();
                let remainder = &content[key_start..];
                let key_end = remainder
                    .find("\r\n")
                    .ok_or(RoutingError::InvalidPacketFormat)?;
                let key_slice = &remainder[..key_end];
                if key_slice.len() != 24 {
                    return Err(RoutingError::InvalidPacketFormat);
                }
                let key: heapless::String<24> = heapless::String::try_from(key_slice)
                    .map_err(|_| RoutingError::InvalidPacketFormat)?;
            ...
            Err(RoutingError::UnsupportedHTTPMethod)
        }
    }
}
use sha1::Digest;
pub async fn approve_web_socket<const N: usize>(
    ...
    let converted_key =
        core::str::from_utf8(&b64_buf[..encoded_len]).expect("invalid b64 encoded buffer");
    ...
    if response
        ...
        || response.push_str("\r\n\r\n").is_err()
    {
        return Err(RoutingError::BufferOverflow);
    }

    socket
        .write_all(response.as_bytes())
        .await
        .map_err(|e| RoutingError::UpgradeRequestFailed(e))?;
...
pub async fn read_ws(socket: &mut TcpSocket<'_>) {
...
fn decode_payload(buf: &mut [u8; 1024], len: usize) -> Result<&str, RoutingError> {
    ...
    let payload_start = match buf[1] & 0x7F {
        ...
        126 => M_PAYLOAD_START,
        127 => L_PAYLOAD_START,
        _ => return Err(RoutingError::InvalidPacketFormat),
    };
    ...
    let payload = &mut buf[payload_start..len];
    let content = core::str::from_utf8(payload).map_err(|_| RoutingError::InvalidUTF8)?;
    Ok(content)
}
